...
Rectangle{
    ...
    */
    ...
    signal sigDeleteClickEditSchedule()
    ...
    Column{
        ...
        TitleBar{
            id:titleBar
            state:"C"
            left_1st_Text:"Edit Schedule"
            onSigLClickTitleBar: {
                initScheduleAct()
                sigBackClickEditSchedule()
            }
            onSigRowLClick: {
                sigDeleteClickEditSchedule()
            }
            onSigRowRClick: {
                //schedule DB에 신호 보내서 수정
                //activation 정보도 실어서 보내기
                ...
                //sigDoneClickEditSchedule(var somedata) *to Do device에 맞게 수정
            }
        }

        List{
            id:scheduleActoption
            left_1st_Text:"Schedule Activation"
            state:"C"
            imagename: "on"
            onSigtoggleOff: {
    ...
    Rectangle{
        ...
        PopupPicker{
            ...
            onSigOkClick: {
                ...
                minText2=_secondText.toString().padStart(2, '0')
    ...
    ListModel{
        id:circuitmodel
        ListElement{listtitle:"Time";Textopacity:1}
        ListElement{listtitle:"Repeat Schedule";Textopacity:1}
        ListElement{listtitle:"Operation Settings";Textopacity:1}
    }
    ListModel{
        id:recircuitmodel
        ListElement{listtitle:"Start Time";Textopacity:1}
        ListElement{listtitle:"End Time";Textopacity:1}
        ListElement{listtitle:"Repeat Schedule";Textopacity:1}
    }
    ListModel{
        id:sliencemodel
    ...
    function setSilentedit(_sthour,_stmin,_stampm,_endhour,_endmin,_endampm){
        hourText=_sthour
        minText=_stmin
        ampmText=_stampm
        ...
        sndText=_endhour+":"+_endmin+" "+_endampm

        var _ampm

        if(_stampm==="AM"){_ampm=0}
        else if(_stampm==="PM"){_ampm=1}
        timePicker.sigSetDefaultIndex(_sthour,_stmin,_ampm)

        if(_endampm==="AM"){_ampm=0}
        else if(_endampm==="PM"){_ampm=1}
    ...
    function seteditSchedule(_hour,_min,_ampm,_runmode,_temp,_days,_isUse){
        hourText=_hour
        minText=_min
        ampmText=_ampm
    ...
    onSigmodelchange:{
        ...
        case "circuit_silent_mode":{themodel=sliencemodel;break;}
        }
    }
}
