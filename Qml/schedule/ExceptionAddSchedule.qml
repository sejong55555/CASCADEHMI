...
Rectangle {
    ...
    signal sigBackClickExAdd()
    signal sigDoneClickExAdd(string _title,string _stYear,string _stMonth,string _stDate,string _endYear,string _endMonth,string _endDate,string _repeat)

    Column{
        anchors.fill:parent
        TitleBar{
            id:titleBar
            left_1st_Text:"Add Exception Day"
            state:"A"
            TitleBarButton{
                x:414;y:11
                imagename:"done"
                onSigClick: {
                    ...
                    initEx()
                    sigDoneClickExAdd(_titleText,_startYear,_startMonth,_startDate,_endYear,_endMonth,_endDate,_repeatopt)
                    popuptoastText="A exception day has been added."
